...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    items.handleGetResurfacedItems()
  }, [])
  ...
  <div className="space-y-8">
    ...
    <nav className="flex-grow space-y-2">
      ...
      <button
      onClick={()=>logutSub()}
      className='bg-red-500 p-2 border-none rounded-xl ml-1 hover:bg-red-700 active:scale-95 text-white'>
        Log Out
      </button>
    </nav>
  </div>
        ...
        {resurfacedItems && resurfacedItems.length > 0 && (
          <section className="mb-8">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="text-white font-semibold">From your memory</h3>
                <p className="text-gray-500 text-sm">Items you saved in the past</p>
              </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {resurfacedItems.map((item) => (
                <div key={item._id} className="bg-[#161926] border border-white/5 rounded-xl p-4 hover:border-purple-500/30 transition">
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex-1">
                      <h4 className="text-white font-medium text-sm mb-1 line-clamp-2">{item.title}</h4>
                      ...
                      {item.tags && item.tags.length > 0 && (
                        <div className="flex flex-wrap gap-1">
                          {item.tags.slice(0, 3).map((tag, index) => (
                            <span key={index} className="px-2 py-0.5 bg-purple-500/10 border border-purple-500/20 text-purple-400 rounded text-xs">
                              {tag}
                            </span>
                          ))}
                        </div>
                    ...
                    <div className="ml-2">
                      {item.type === 'Video' && <Play size={16} className="text-gray-400" />}
                      {item.type === 'Article' && <FileText size={16} className="text-gray-400" />}
                      {item.type === 'Image' && <ImageIcon size={16} className="text-gray-400" />}
                      {item.type === 'Tweet' && <MessageSquare size={16} className="text-gray-400" />}
                      {item.type === 'Pdf' && <FileText size={16} className="text-gray-400" />}
                    </div>
                  </div>
                  <a
                    href={item.url}
